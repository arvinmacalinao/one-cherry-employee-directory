            ...
            <tbody>
                @forelse ($offices as $office)
                    <tr class="border-b border-line last:border-b-0 hover:bg-surface">
                        ...
                        <td class="px-4 py-3 text-ink-secondary">{{ $office->address ?: '—' }}</td>
                ...
                @empty
                    <tr><td colspan="5" class="px-4 py-10 text-center text-ink-secondary">No office locations match.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    ...
    @if ($showForm)
        <x-admin.drawer :title="$editingId ? 'Edit Office Location' : 'Add Office Location'">
            <div class="mb-4 flex items-start gap-2.5 rounded-lg bg-brand-tint px-3.5 py-3 text-xs text-ink-secondary">
                ...
                <span>Office Locations are fully Directory-managed — HR does not track this, so nothing here is ever overwritten by sync.</span>
            </div>
