...
**Identity + review pattern for Company/Designation** (Department is exempt — see above, it's never sync-created): matching now prefers HR's numeric `company_id`/`designation_id` (via `companies.hr_ref_id` / `designations.hr_ref_id`) when the API sends them, falling back to a case-insensitive name match when it doesn't — see `HrSyncService::resolveCompany()`/`resolveDesignation()`. A `needs_review` boolean (replacing an earlier numeric-ID-based `"Unmapped X #<id>"` naming convention that no longer applies once real names are available) is set `true` when sync auto-creates a Company or Designation it hasn't seen before under either identity, and is cleared the moment an Admin opens and saves that record from `/admin/companies` or `/admin/designations` — surfaced as a sidebar badge and on `/admin/sync`, with both a "Merge into existing" action (for when HR sends a near-duplicate name, e.g. a typo or rename) and a "Mark Reviewed" action (when the new name is simply legitimate and just needs its branding/hierarchy filled in).

**Renaming no longer breaks re-matching, once HR sends the ID.** The name-only-matching gap described below is fixed for any Company/Designation whose `hr_ref_id` is populated — an Admin can freely rename it in OCED and the next sync still recognizes it by ID. The one remaining edge case: a record whose `hr_ref_id` is still null (never matched by ID yet) is renamed in OCED *before* its first ID-bearing sync — that still falls through to a name-based lookup, misses, and auto-creates a duplicate. `resolveCompany()`/`resolveDesignation()` mitigate this by opportunistically backfilling `hr_ref_id` the moment a name match succeeds, so this gap only affects a record between its creation and its first successful sync — not on every sync thereafter.
...
**Implemented: ID-based re-matching.** HR's `/api/employees` controller now adds `company_id` and `designation_id` (the raw numeric FK values off their `users` table) alongside the existing `company`/`designation` name strings — same endpoint, two more fields. `HrEmployeeData` carries both as nullable ints, `HrRestApiSource` maps them when present, and `HrSyncService::resolveCompany()`/`resolveDesignation()` match on `companies.hr_ref_id` / `designations.hr_ref_id` first, falling back to case-insensitive name matching (and backfilling `hr_ref_id` onto the match) when the ID is absent or unmatched — so this degrades gracefully against an HR response that hasn't been updated yet, and no coordinated two-sided cutover was needed.
...
- ✅ Company/Designation identity resolved by **HR's numeric `company_id`/`designation_id`** when sent, falling back to name match — auto-creates with `needs_review=true` on an identity never seen before, reviewed/merged from `/admin/sync` (§2.4).
...
4. **Sensitive field visibility** — should Personal Email/Emergency Contact be visible to all employees, or restricted to the employee + their manager + Admin? (§10)
