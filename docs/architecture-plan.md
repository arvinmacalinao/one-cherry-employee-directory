...
- `HrSyncService` resolves `employees.employee_status_id` by matching `employment_status.id` against `employee_statuses.hr_ref_id`, auto-creating a new row the first time an unseen `es_id` appears. There is no fixed list, no enum, no bucket OCED decides on — whatever HR calls a status is what OCED displays, verbatim, forever. If HR renames a status or adds a new one, the next sync just reflects it.
